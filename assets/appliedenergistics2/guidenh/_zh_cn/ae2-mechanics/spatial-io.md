...
作用范围的选取由空间塔方块定义,所有空间塔必须在同一个AE网络中，建议使用[子网](./subnetworks.md)。使用1\*1\*n（n>1）个相邻的空间塔方块可以构造一个空间柱，多个空间柱将会构造一个厚度为1格的“方形壳”，其以内的空间即为空间IO系统的作用范围，也就是说空间IO的作用范围比整个空间塔多方块结构的范围“小一圈”。

空间IO系统多方块构建遵守以下规则：
  * 每个空间柱至少由2个空间塔方块构成，空间柱只能呈直线。
  * 所有空间柱必须在同一个AE网络中，每个空间柱占用1个频道。
